...
import { destinationCreatedSuccessfully } from "../../../../Utils/ToastFunctions";
...
const AddDestination = () => {
  ...
  // ! function for creating a destination
  const handleAddDestination = (data) => {
    ...
    axiosPrivateUrl
      .post("/api/destination", destinationData)
      .then((response) => {
        console.log(response?.data);
        if (response?.data) {
          destinationCreatedSuccessfully();
          reset();
        }
      })
      .catch((error) => {
        console.log("error in add deestination ");
        console.log(error);
      });
  };
  ...
  return (
    <div className="AddDestinationContainer">
      <ToastContainer />
      <div className="AddDestinationWrapper h-screen flex justify-center items-center ">
        {/* add destination card starts  */}
        <div className="addDestinationCard bg-white  shadow-2xl  py-9 px-4 w-[94%] xsm:w-[90%] sm:w-[88%] md:w-[86%] xmd:w-[78%] lg:w-[70%] rounded-md border border-gray-300 ">
          <h1 className="mb-4 text-xl font-bold text-center  xsm:text-2xl md:text-3xl sm:mb-6 md:mb-8 lg:mb-10">
            Add Destination
          </h1>

          <form
            onSubmit={handleSubmit(handleAddDestination)}
